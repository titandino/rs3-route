...
public final class AreaDefinitions {
	...
	public static final AreaDefinitions getDefinitions(int id) {
		...
		if (area != null)
			return area;
		byte[] data = Cache.STORE.getIndex(IndexType.CONFIG).getFile(FileType.MAP_AREAS.getId(), id);
	...
	public static void dump() throws IOException {
		...
		writer.append("//Version = 905\n");
		writer.flush();
		for (int i = 0;i < Cache.STORE.getIndex(IndexType.CONFIG).getLastFileId(FileType.MAP_AREAS.getId())+1;i++) {
